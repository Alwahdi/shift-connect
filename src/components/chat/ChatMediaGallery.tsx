...
import { Image, File, Link as LinkIcon, Download, ExternalLink, X } from "lucide-react";
...
export const ChatMediaGallery = ({ conversationId, isOpen, onClose }: ChatMediaGalleryProps) => {
  ...
  const images = mediaItems.filter((item) => item.file_type?.startsWith("image/"));
  const files = mediaItems.filter((item) => item.file_type && !item.file_type.startsWith("image/"));

  // Extract links from message content
  const extractLinks = (): { url: string; content: string; created_at: string }[] => {
    ...
    mediaItems.forEach((item) => {
      if (item.content) {
        ...
        if (matches) {
          matches.forEach((url) => {
            links.push({ url, content: item.content, created_at: item.created_at });
          });
        }
      }
    });
        ...
        <DialogContent className="sm:max-w-3xl p-0 overflow-hidden">
          ...
          <div className="relative">
            <Button
              variant="ghost"
              size="icon"
              className="absolute top-2 end-2 z-10 bg-background/80 hover:bg-background"
