...
export function EditShiftSheet({ visible, shift, onClose, onUpdated }: Props) {
  const { control, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm<FormValues>({
    ...
    values: {
      ...
      required_certifications: shift.required_certifications?.join(', ') ?? '',
    },
  });
  const [urgent, setUrgent] = React.useState(Boolean(shift.is_urgent));
  ...
  const submit = handleSubmit(async (values) => {
    try {
      ...
      onUpdated?.();
      onClose();
      Alert.alert('Shift updated', 'Your shift has been saved.');
    } catch (error) {
      Alert.alert('Unable to update shift', error instanceof Error ? error.message : 'Please try again.');
    }
  });
            ...
            <Card style={styles.formCard}>
              ...
              <View style={styles.switchRow}>
                <View>
                  ...
                  <Text style={styles.switchHelp}>Highlight for faster professional responses.</Text>
                </View>
                <Switch value={urgent} onValueChange={setUrgent} trackColor={{ true: theme.colors.primary }} />
              </View>

              <Button title="Save changes" fullWidth onPress={submit} loading={isSubmitting} />
            </Card>
          </ScrollView>
        </View>
      </View>
